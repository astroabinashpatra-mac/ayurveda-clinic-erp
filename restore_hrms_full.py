...
# 1. Complete 4-Section HRMS Modal HTML
full_hrms_modal = '''  <!-- FULL 4-SECTION EMPLOYEE ONBOARDING MODAL -->
...
# Replace modal HTML block safely using regex pattern
html = re.sub(r'<div id="md-emp".*?</div>\s*<!--', full_hrms_modal + '\n\n  <!--', html, flags=re.DOTALL)

# 2. JavaScript HRMS Functions
js_hrms_functions = '''    function renderEmployees() {
    ...
    function openEmployeeModal() {
      ...
      if (document.getElementById('title-emp-modal')) document.getElementById('title-emp-modal').innerText = '👤 Complete Employee Onboarding Registration';
      openModal('md-emp');
    }
    ...
    function editEmployee(id) {
      ...
      if (document.getElementById('title-emp-modal')) document.getElementById('title-emp-modal').innerText = '✏️ Edit Employee Profile';
      openModal('md-emp');
    }
...
# Replace HRMS JS functions in index.html
if 'function renderEmployees()' in html:
    start_pos = html.find('function renderEmployees()')
    end_pos = html.find('async function deboardEmp(id)')
    if start_pos != -1 and end_pos != -1:
        html = html[:start_pos] + js_hrms_functions + '\n\n    ' + html[end_pos:]

# Update HRMS Onboard button target to openEmployeeModal()
html = html.replace("onclick=\"openModal('md-emp')\"", "onclick=\"openEmployeeModal()\"")

with open("index.html", "w", encoding="utf-8") as f:
    f.write(html)

print("Employee HRMS registration form and storage routines fully restored.")
